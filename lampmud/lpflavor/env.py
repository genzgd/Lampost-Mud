...
def find_cost(room):
    if room and room.size:
        try:
            ...
            exit_cost_map[room.size] = {'action': int(action_calc * room.size / lampmud.env.room.default_room_size),
                                        'stamina': int(stamina_calc * room.size / lampmud.env.room.default_room_size)}
            return exit_cost_map[room.size]
